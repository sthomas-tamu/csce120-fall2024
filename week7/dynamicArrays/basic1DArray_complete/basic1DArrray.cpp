...
//double max(const double* nums, unsigned int size); // can also use double* double[]
...
int main() {
    ...
    for (unsigned int i=0; i<size; ++i) {
        cin >> ary[i]; // do this
        //cin >> *(ary + i); // don't do this, works but is hard to read/debug
        if (cin.fail()) {
...
double max(const double nums[], unsigned int size) {
//double max(const double* nums, unsigned int size) { // can also use double* instead of double[]
    if (size == 0) {
